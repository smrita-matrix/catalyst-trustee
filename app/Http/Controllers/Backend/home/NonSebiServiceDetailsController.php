...
class NonSebiServiceDetailsController extends Controller
{
    /**
    ...
    {
        $non_sebi_services = NonSebiServiceDetails::whereNull('deleted_at')
            ->orderBy('id', 'desc')
            ->get();

        return view('backend.home-page.non-sebi-service-details.index', compact('non_sebi_services'));
    }

    public function create(Request $request)
    {
        return view('backend.home-page.non-sebi-service-details.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('non-sebi-service-details.index')->with('message', 'Non-SEBI Services section added successfully!');
    }

    public function edit($id)
    {
        $non_sebi_service = NonSebiServiceDetails::findOrFail($id);

        return view('backend.home-page.non-sebi-service-details.edit', compact('non_sebi_service'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('non-sebi-service-details.index')->with('message', 'Non-SEBI Services section has been successfully updated!');
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('non-sebi-service-details.index')->with('message', 'Non-SEBI Services section deleted successfully!');
        } catch (\Exception $ex) {
    ...
    {
        $destination = public_path('home/non-sebi-services');

        if (!is_dir($destination)) {
            mkdir($destination, 0775, true);
        }

        $fileName = 'nonsebi_' . time() . '_' . Str::random(8) . '.' . strtolower($file->getClientOriginalExtension());
    ...
    {
        $path = public_path('home/non-sebi-services/' . $fileName);
        if (is_file($path)) {
            @unlink($path);
        }
    }
