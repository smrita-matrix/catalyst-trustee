...
class ServiceLayout3Controller extends Controller
{
    ...
    private function rules()
    {
        return [
            ...
            'benefits_image'          => 'nullable|image|mimes:jpg,jpeg,png,webp|max:8192',
            'tab_icon.*'              => 'nullable|image|mimes:jpg,jpeg,png,webp,svg|max:8192',
        ];
    }
    ...
    private function messages()
    {
        return [
            ...
            'benefits_image.max'          => 'The image must not be larger than 8MB.',
        ];
    }
    ...
    {
        $destination = public_path('service-uploads/layout3/' . $folder);
    ...
    {
        $path = public_path('service-uploads/layout3/' . $folder . '/' . $fileName);
        if (is_file($path)) {
            @unlink($path);
        }
    }
