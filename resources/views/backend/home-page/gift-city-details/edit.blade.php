            ...
            <div class="page-title">
              <div class="row">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit GIFT City</li>
                </ol>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        ...
                        <p class="f-m-light mt-1">Update the section heading, its service items, and the bottom text.</p>
                    </div>
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('gift-city-details.update', $gift_city->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="heading" type="text" name="heading" value="{{ old('heading', $gift_city->heading) }}" placeholder="e.g. GIFT City Service Offerings" required>
                                        ...
                                        <tbody id="items-tbody">
                                            @php
                                                $existingItems = old('item_title')
                                                    ...
                                                    : ($gift_city->items ?: [['title' => '', 'title_link' => '', 'description' => '', 'image' => null]]);
                                            @endphp
                                                ...
                                                <tr class="item-row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <div class="img-preview item-image-preview">
                                                            @if (!empty($item['image']))
                                                                <img src="{{ asset('home/gift-city/' . $item['image']) }}" alt="image">
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <textarea class="form-control" name="item_title[]" rows="2" placeholder="e.g. Alternative Investment Funds (AIFs) in GIFT City">{{ $item['title'] ?? '' }}</textarea>
                                                    </td>
                                                    <td>
                                                        <input class="form-control" type="text" name="item_title_link[]" value="{{ $item['title_link'] ?? '' }}" placeholder="e.g. #">
                                                    </td>
                                                    <td>
                                                        <textarea class="form-control" name="item_description[]" rows="2" placeholder="Item paragraph">{{ $item['description'] ?? '' }}</textarea>
                                                    </td>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="footer_text" name="footer_text" rows="3" placeholder="e.g. As GIFT City continues to evolve as a global financial destination...">{{ old('footer_text', $gift_city->footer_text) }}</textarea>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Live image preview (replaces the shown current image)
