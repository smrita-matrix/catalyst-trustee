        ...
        <div class="page-body">
          <div class="container-fluid">
            <div class="page-title">
              <div class="row">
                <div class="col-6">
                  <h4>Edit Business Performance</h4>
                </div>
                <div class="col-6">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit Business Performance</li>
                </ol>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        ...
                        <p class="f-m-light mt-1">Update the headings, categories (legend + colors) and the yearly transaction values.</p>
                    </div>
                    <div class="card-body">
                        @php
                            $defaultLabels = ['Debenture Trustee', 'Security Trustee', 'Securitization', 'Others'];
                            $defaultColors = ['#c9624c', '#4a7fb5', '#e8a838', '#3d8c6f'];
                            $storedCats = $business_performance->categories ?: [];
                            $catLabels = old('cat_label', count($storedCats) ? array_column($storedCats, 'label') : $defaultLabels);
                            $catColors = old('cat_color', count($storedCats) ? array_column($storedCats, 'color') : $defaultColors);
                            $catCount  = count($catLabels);

                            $storedYears = $business_performance->years ?: [];
                            $rows = [];
                            if (old('year')) {
                                foreach (old('year') as $r => $y) {
                                    $vals = [];
                                    for ($c = 0; $c < $catCount; $c++) {
                                        $vals[$c] = old('value')[$c][$r] ?? '';
                                    }
                                    $rows[] = ['year' => $y, 'values' => $vals];
                                }
                            } else {
                                $rows = count($storedYears) ? $storedYears : [['year' => '', 'values' => array_fill(0, $catCount, '')]];
                            }
                        @endphp

                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('business-performance-details.update', $business_performance->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-lg-6">
                                ...
                                <input class="form-control" id="sub_heading" type="text" name="sub_heading" value="{{ old('sub_heading', $business_performance->sub_heading) }}" placeholder="e.g. Catalyst delivers more than expected">
                            </div>

                            <!-- Heading -->
                            <div class="col-lg-6">
                                <label class="form-label" for="heading">Heading</label>
                                <input class="form-control" id="heading" type="text" name="heading" value="{{ old('heading', $business_performance->heading) }}" placeholder="e.g. Business Performance">
                            </div>
                                        ...
                                        <tbody id="years-tbody">
                                            @foreach ($rows as $r => $row)
                                                <tr class="year-row">
                                                    ...
                                                    <td><input class="form-control" type="text" name="year[]" value="{{ $row['year'] ?? '' }}" placeholder="e.g. 2016-17"></td>
                                                    @for ($c = 0; $c < $catCount; $c++)
                                                        <td><input class="form-control year-value" type="number" step="any" name="value[{{ $c }}][]" value="{{ $row['values'][$c] ?? '' }}" placeholder="0"></td>
                                                    @endfor
