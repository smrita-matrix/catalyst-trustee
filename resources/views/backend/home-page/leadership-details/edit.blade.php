            ...
            <div class="page-title">
              <div class="row">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit Leadership</li>
                </ol>
                </div>
              </div>
            </div>
          ...
          <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('leadership-details.update', $leadership->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            @php
                                $leaders = old('leader_name')
                                    ? collect(old('leader_name'))->map(function ($n, $i) {
                                        return [
                                            'name'        => $n,
                                            'designation' => old('leader_designation')[$i] ?? '',
                                            'description' => old('leader_description')[$i] ?? '',
                                            'image'       => old('leader_existing_image')[$i] ?? null,
                                        ];
                                    })->all()
                                    : ($leadership->leaders ?: [['name' => '', 'designation' => '', 'description' => '', 'image' => null]]);

                                $numbers = old('number_text')
                                    ? collect(old('number_text'))->map(function ($t, $i) {
                                        return [
                                            'count_text' => $t,
                                            'number'     => old('number_value')[$i] ?? '',
                                            'suffix'     => old('number_suffix')[$i] ?? '',
                                            'icon'       => old('number_existing_icon')[$i] ?? null,
                                        ];
                                    })->all()
                                    : ($leadership->numbers ?: [['count_text' => '', 'number' => '', 'suffix' => '', 'icon' => null]]);
                            @endphp

                            <!-- ============ Our Leadership ============ -->
                            <div class="col-12">
                                <label class="form-label" for="leadership_heading">Leadership Heading</label>
                                <input class="form-control" id="leadership_heading" type="text" name="leadership_heading" value="{{ old('leadership_heading', $leadership->leadership_heading) }}" placeholder="e.g. Our Leadership">
                            </div>
                                            ...
                                            @foreach ($leaders as $i => $leader)
                                                <tr class="leader-row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="hidden" name="leader_existing_image[]" value="{{ $leader['image'] ?? '' }}">
                                                        <div class="img-preview leader-image-preview">
                                                            @if (!empty($leader['image']))
                                                                <img src="{{ asset('home/leadership/' . $leader['image']) }}" alt="photo">
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td><input class="form-control" type="text" name="leader_name[]" value="{{ $leader['name'] ?? '' }}" placeholder="e.g. Mr. Chintaman Dixit"></td>
                                                    <td><input class="form-control" type="text" name="leader_designation[]" value="{{ $leader['designation'] ?? '' }}" placeholder="e.g. Founder"></td>
                                                    <td><textarea class="form-control leader-editor" name="leader_description[]" placeholder="Leadership bio / description">{{ $leader['description'] ?? '' }}</textarea></td>
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="numbers_heading" type="text" name="numbers_heading" value="{{ old('numbers_heading', $leadership->numbers_heading) }}" placeholder="e.g. Our Journey In Numbers">
                            </div>
                            ...
                            <div class="col-12">
                                            ...
                                            @foreach ($numbers as $i => $number)
                                                <tr class="number-row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="hidden" name="number_existing_icon[]" value="{{ $number['icon'] ?? '' }}">
                                                        <div class="img-preview number-icon-preview">
                                                            @if (!empty($number['icon']))
                                                                <img src="{{ asset('home/leadership/' . $number['icon']) }}" alt="icon">
                                                            @endif
                                                        </div>
                                                    </td>
                                                    <td><input class="form-control" type="text" name="number_text[]" value="{{ $number['count_text'] ?? '' }}" placeholder="e.g. No. Of Clients"></td>
                                                    <td><input class="form-control" type="text" name="number_value[]" value="{{ $number['number'] ?? '' }}" placeholder="e.g. 1000"></td>
                                                    <td><input class="form-control" type="text" name="number_suffix[]" value="{{ $number['suffix'] ?? '' }}" placeholder="e.g. + or Cr+"></td>
                                ...
                                <small class="text-secondary"><i class="fa fa-info-circle"></i> Leave a file empty to keep the current image. Serial numbers (01, 02…) are added automatically. Empty rows are ignored.</small>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function leaderTemplate() {
            ...
            row.innerHTML =
                ...
                '<td><input class="form-control mb-2 leader-image-input" type="file" name="leader_image[]" accept=".png, .jpg, .jpeg, .webp, .svg"><input type="hidden" name="leader_existing_image[]" value=""><div class="img-preview leader-image-preview"></div></td>' +
        ...
        leadersTbody.addEventListener('click', function (e) {
            ...
            if (rows.length > 1) {
                ...
                row.querySelector('input[name="leader_existing_image[]"]').value = '';
        ...
        function numberTemplate() {
            ...
            row.innerHTML =
                '<td class="row-index"></td>' +
                '<td><input class="form-control mb-2 number-icon-input" type="file" name="number_icon[]" accept=".png, .jpg, .jpeg, .webp, .svg"><input type="hidden" name="number_existing_icon[]" value=""><div class="img-preview number-icon-preview"></div></td>' +
        ...
        numbersTbody.addEventListener('click', function (e) {
            ...
            if (rows.length > 1) {
                ...
                row.querySelector('input[name="number_existing_icon[]"]').value = '';
