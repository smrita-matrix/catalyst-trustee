        ...
        <div class="page-body">
          <div class="container-fluid">
            <div class="page-title">
              <div class="row">
                <div class="col-6">
                  <h4>Edit Activities Outside SEBI Regulations Section</h4>
                </div>
                <div class="col-6">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit Activities Outside SEBI Regulations</li>
                </ol>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        <h4>Activities Outside the Purview of SEBI Regulations</h4>
                        <p class="f-m-light mt-1">Update the section heading and its service cards.</p>
                    </div>
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('non-sebi-service-details.update', $non_sebi_service->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="heading" type="text" name="heading" value="{{ old('heading', $non_sebi_service->heading) }}" placeholder="e.g. Activities Outside the Purview of SEBI Regulations" required>
                                    ...
                                    @php
                                        $existingItems = old('item_title')
                                            ? collect(old('item_title'))->map(function ($t, $i) {
                                                return [
                                                    'title'          => $t,
                                                    'title_link'     => old('item_title_link')[$i] ?? '',
                                                    'description'    => old('item_description')[$i] ?? '',
                                                    'read_more_link' => old('item_read_more_link')[$i] ?? '',
                                                    'service_img'    => old('item_existing_service_img')[$i] ?? null,
                                                    'icon'           => old('item_existing_icon')[$i] ?? null,
                                                ];
                                            })->all()
                                            : ($non_sebi_service->items ?: [['title' => '', 'title_link' => '', 'description' => '', 'read_more_link' => '', 'service_img' => null, 'icon' => null]]);
                                    @endphp
                                        ...
                                        <div class="item-block border rounded p-3 mb-3 position-relative">
                                            ...
                                            <div class="row g-3">
                                                <div class="col-lg-6">
                                                    ...
                                                    <input type="hidden" name="item_existing_service_img[]" value="{{ $item['service_img'] ?? '' }}">
                                                    <div class="img-preview mt-2 service-img-preview">
                                                        @if (!empty($item['service_img']))
                                                            <img src="{{ asset('home/non-sebi-services/' . $item['service_img']) }}" alt="service image">
                                                        @endif
                                                    </div>
                                                </div>
                                                <div class="col-lg-6">
                                                    ...
                                                    <div class="img-preview mt-2 icon-preview">
                                                        @if (!empty($item['icon']))
                                                            <img src="{{ asset('home/non-sebi-services/' . $item['icon']) }}" alt="icon">
                                                        @endif
                                                    </div>
                                                </div>
                                                <div class="col-lg-6">
                                                    <label class="form-label">Title</label>
                                                    <input class="form-control" type="text" name="item_title[]" value="{{ $item['title'] ?? '' }}" placeholder="e.g. Debenture Trustee Services (Listed)">
                                                </div>
                                                <div class="col-lg-6">
                                                    <label class="form-label">Title Link</label>
                                                    <input class="form-control" type="text" name="item_title_link[]" value="{{ $item['title_link'] ?? '' }}" placeholder="e.g. #">
                                                </div>
                                                <div class="col-12">
                                                    <label class="form-label">Description</label>
                                                    <textarea class="form-control" name="item_description[]" rows="3" placeholder="Card description">{{ $item['description'] ?? '' }}</textarea>
                                                </div>
                                                <div class="col-lg-6">
                                                    <label class="form-label">Read More Link</label>
                                                    <input class="form-control" type="text" name="item_read_more_link[]" value="{{ $item['read_more_link'] ?? '' }}" placeholder="e.g. #">
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                <small class="text-secondary"><i class="fa fa-info-circle"></i> Leave a file empty to keep the current image. Max 2MB each. Empty cards are ignored.</small>
                            </div>

                            <!-- Form Actions -->
                            <div class="col-12 d-flex justify-content-end gap-2 border-top pt-4 mt-2">
                                <a href="{{ route('non-sebi-service-details.index') }}" class="btn btn-outline-secondary px-4">Cancel</a>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function blockTemplate() {
            ...
            block.innerHTML =
                ...
                '</div>' +
                '<div class="row g-3">' +
                    '<div class="col-lg-6"><label class="form-label">Service Image</label><input class="form-control service-img-input" type="file" name="item_service_img[]" accept=".png, .jpg, .jpeg, .webp, .svg"><input type="hidden" name="item_existing_service_img[]" value=""><div class="img-preview mt-2 service-img-preview"></div></div>' +
                    '<div class="col-lg-6"><label class="form-label">Icon</label><input class="form-control icon-input" type="file" name="item_icon[]" accept=".png, .jpg, .jpeg, .webp, .svg"><input type="hidden" name="item_existing_icon[]" value=""><div class="img-preview mt-2 icon-preview"></div></div>' +
        ...
        wrapper.addEventListener('click', function (e) {
            ...
            if (blocks.length > 1) {
                ...
                block.querySelectorAll('input[type="hidden"]').forEach(function (el) { el.value = ''; });
                block.querySelectorAll('.img-preview').forEach(function (p) { p.innerHTML = ''; });
            }
            reindex();
        });

        // Live image preview (service image + icon), replacing the shown current image
