            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        <h4>Footer</h4>
                        <p class="f-m-light mt-1">Update the footer logo, description, contact details and social links.</p>
                    </div>
                    <div class="card-body">
                        @php
                            $socialOptions = [
                            ...
                            if (old('social_icon')) {
                                $socialRows = collect(old('social_icon'))->map(function ($icon, $i) {
                                    return ['icon' => $icon, 'url' => old('social_url')[$i] ?? ''];
                                })->all();
                            } else {
                                $socialRows = $footer->social_links ?: [['icon' => 'fa fa-facebook', 'url' => '']];
                            }
                        @endphp

                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('footer-details.update', $footer->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-lg-6">
                                <label class="form-label">{{ $footer->logo ? 'Current Logo' : 'Logo Preview' }}</label>
                                <div class="img-preview logo-preview">
                                    @if ($footer->logo)
                                        <img id="existing_logo" src="{{ asset('home/footer/' . $footer->logo) }}" alt="logo">
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3" placeholder="e.g. Catalyst Trusteeship Limited is one of the leading providers...">{{ old('description', $footer->description) }}</textarea>
                            </div>

                            <!-- Phone -->
                            <div class="col-lg-4">
                                <label class="form-label" for="phone">Phone</label>
                                <input class="form-control" id="phone" type="text" name="phone" value="{{ old('phone', $footer->phone) }}" placeholder="e.g. +91 (022) 4922 0555">
                            </div>

                            <!-- Email -->
                            <div class="col-lg-4">
                                <label class="form-label" for="email">Email</label>
                                <input class="form-control" id="email" type="text" name="email" value="{{ old('email', $footer->email) }}" placeholder="e.g. dt.mumbai@ctltrustee.com">
                            </div>

                            <!-- Address -->
                            <div class="col-lg-4">
                                <label class="form-label" for="address">Address</label>
                                <textarea class="form-control" id="address" name="address" rows="1" placeholder="e.g. 901, 9th Floor, Towr-B, Peninsula Business Park...">{{ old('address', $footer->address) }}</textarea>
                            </div>
                                                    ...
                                                    <td>
                                                        <select class="form-control" name="social_icon[]">
                                                            @foreach ($socialOptions as $val => $lbl)
                                                                <option value="{{ $val }}" {{ ($row['icon'] ?? '') === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td><input class="form-control" type="text" name="social_url[]" value="{{ $row['url'] ?? '' }}" placeholder="https://..."></td>
