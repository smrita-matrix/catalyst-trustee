            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        ...
                        <p class="f-m-light mt-1">Update the section heading and its items (icon + paragraph).</p>
                    </div>
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('why-choose-details.update', $why_choose->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="heading" type="text" name="heading" value="{{ old('heading', $why_choose->heading) }}" placeholder="e.g. Why Choose Catalyst Trusteeship Limited?" required>
                            ...
                            <div class="col-12">
                                    ...
                                    <table class="table table-bordered align-middle" id="items-table">
                                        <thead class="table-light">
                                            <tr>
                                                ...
                                                <th style="width: 320px;">Icon (upload or paste SVG)</th>
                                                <th>Paragraph</th>
                                        ...
                                        <tbody id="items-tbody">
                                            @php
                                                $existingItems = old('item_text')
                                                    ...
                                                    : ($why_choose->items ?: [['text' => '', 'icon_svg' => null, 'icon' => null]]);
                                            @endphp
                                            @foreach ($existingItems as $i => $item)
                                                <tr class="item-row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <textarea class="form-control item-svg-input mb-2" name="item_icon_svg[]" rows="2" placeholder="…or paste &lt;svg&gt;…&lt;/svg&gt; code">{{ $item['icon_svg'] ?? '' }}</textarea>
                                                        <div class="item-current-icon">
                                                            @if (!empty($item['icon_svg']))
                                                                <span style="display:inline-block; max-height:42px; max-width:42px; overflow:hidden;">{!! $item['icon_svg'] !!}</span>
                                                            @elseif (!empty($item['icon']))
                                                                <img src="{{ asset('home/why-choose/' . $item['icon']) }}" alt="icon" style="max-height: 42px; max-width: 42px; border: 1px solid #eee; padding: 2px; border-radius: 4px;">
                                                            @else
                                                    ...
                                                    <td>
                                                        <textarea class="form-control" name="item_text[]" rows="2" placeholder="Paragraph text">{{ $item['text'] ?? '' }}</textarea>
                                                    </td>
                                ...
                                <small class="text-secondary"><i class="fa fa-info-circle"></i> Leave the file empty to keep a row's current icon. Upload (.svg/.png/.jpg/.webp, max 2MB) or paste raw &lt;svg&gt; code. Empty rows are ignored.</small>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function rowTemplate() {
            ...
            row.innerHTML =
                ...
                '<td>' +
                    '<input class="form-control mb-2 item-icon-input" type="file" name="item_icon[]" accept=".svg, .png, .jpg, .jpeg, .webp">' +
                    '<input type="hidden" name="item_existing_icon[]" value="">' +
                    '<textarea class="form-control item-svg-input mb-2" name="item_icon_svg[]" rows="2" placeholder="…or paste <svg>…</svg> code"></textarea>' +
                    '<div class="item-current-icon"><span class="text-muted small">No icon</span></div>' +
                '</td>' +
        ...
        tbody.addEventListener('click', function (e) {
            ...
            if (rows.length > 1) {
                ...
                row.querySelector('.item-current-icon').innerHTML = '<span class="text-muted small">No icon</span>';
        ...
        tbody.addEventListener('change', function (e) {
            ...
            const box = input.parentElement.querySelector('.item-current-icon');
        ...
        tbody.addEventListener('input', function (e) {
            ...
            const box = svgInput.parentElement.querySelector('.item-current-icon');
            if (!box) return;
