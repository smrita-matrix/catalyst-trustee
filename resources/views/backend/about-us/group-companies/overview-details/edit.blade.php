            ...
            <div class="page-title">
              <div class="row">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Edit Group Overview</li>
                </ol>
                </div>
              </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        ...
                        <p class="f-m-light mt-1">Update the two images, heading and description.</p>
                    </div>
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input banner-form" novalidate action="{{ route('group-companies-overview-details.update', $overview->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="img-preview bg-image-preview mt-2" id="main-preview">
                                    @if ($overview->main_image)<img src="{{ asset('about-us/group-companies/overview/' . $overview->main_image) }}" alt="main">@endif
                                </div>
                            </div>
                            ...
                            <div class="col-lg-6">
                                ...
                                <small class="d-block text-secondary mt-2"><i class="fa fa-info-circle"></i> Leave empty to keep the current image. Max 2MB.</small>
                                <div class="img-preview bg-image-preview mt-2" id="small-preview">
                                    @if ($overview->small_image)<img src="{{ asset('about-us/group-companies/overview/' . $overview->small_image) }}" alt="small">@endif
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <input class="form-control" id="heading" type="text" name="heading" value="{{ old('heading', $overview->heading) }}" placeholder="e.g. Catalyst Group Overview" required>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="editor" name="description" placeholder="Enter the description">{{ old('description', $overview->description) }}</textarea>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="table-responsive custom-scrollbar">
                                    <table class="table table-bordered align-middle" id="entities-table">
                                        ...
                                        <tbody id="entities-tbody">
                                            @php
                                                $entRows = old('entity_title')
                                                    ? collect(old('entity_title'))->map(function ($t, $i) {
                                                        return ['title' => $t, 'description' => old('entity_description')[$i] ?? '', 'link' => old('entity_link')[$i] ?? '', 'image' => old('entity_existing_image')[$i] ?? null];
                                                    })->all()
                                                    : ($overview->entities ?: [['title' => '', 'description' => '', 'link' => '', 'image' => null]]);
                                            @endphp
                                            @foreach ($entRows as $i => $ent)
                                                <tr class="entity-row">
                                                    <td class="row-index">{{ $i + 1 }}</td>
                                                    <td>
                                                        <input class="form-control mb-2 entity-image-input" type="file" name="entity_image[]" accept=".png, .jpg, .jpeg, .webp, .svg">
                                                        <input type="hidden" name="entity_existing_image[]" value="{{ $ent['image'] ?? '' }}">
                                                        <div class="img-preview entity-image-preview">
                                                            @if (!empty($ent['image']))<img src="{{ asset('about-us/group-companies/overview/' . $ent['image']) }}" alt="logo">@endif
                                                        </div>
                                                    </td>
                                                    <td><input class="form-control" type="text" name="entity_title[]" value="{{ $ent['title'] ?? '' }}" placeholder="e.g. CTL Trusteeship Limited"></td>
                                                    <td><textarea class="form-control" name="entity_description[]" rows="3" placeholder="Entity description">{{ $ent['description'] ?? '' }}</textarea></td>
                                                    <td><input class="form-control" type="text" name="entity_link[]" value="{{ $ent['link'] ?? '' }}" placeholder="e.g. #"></td>
                                ...
                                <small class="text-secondary"><i class="fa fa-info-circle"></i> Leave a file empty to keep the current logo. Empty rows are ignored.</small>
                            </div>
