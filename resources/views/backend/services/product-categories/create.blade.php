            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card-header">
                        <h4>Product</h4>
                        <p class="f-m-light mt-1">Add a product under a service category (e.g. Debenture Trustee Services (Listed) under SEBI Regulated Services).</p>
                    </div>
                    <div class="card-body">
                        <form class="row g-4 needs-validation custom-input" novalidate action="{{ route('product-category.store') }}" method="POST">
                            @csrf
                            <div class="col-lg-6">
                                ...
                                <select class="form-select" id="service_category_id" name="service_category_id" required>
                                    ...
                                    @foreach($serviceCategories as $cat)
                                        <option value="{{ $cat->id }}" {{ (string) old('service_category_id') === (string) $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-6">
                                ...
                                <input class="form-control" id="name" type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Debenture Trustee Services (Listed)" required>
                            ...
                            <div class="col-lg-6">
                                ...
                                <select class="form-select" id="layout" name="layout">
                                    ...
                                    @foreach(\App\Models\ProductCategory::LAYOUTS as $key => $label)
                                        <option value="{{ $key }}" {{ old('layout') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="disclaimer" name="disclaimer" rows="3" placeholder="Leave empty if this service needs no disclaimer">{{ old('disclaimer') }}</textarea>
